...
export default p => {
  ...
  beginShape = beginShape.bind(p);
  cos = cos.bind(p);
  sin = sin.bind(p);
  vertex = vertex.bind(p);
  endShape = endShape.bind(p);
  createVector = createVector.bind(p);
  random = random.bind(p);
  noStroke = noStroke.bind(p);
  fill = fill.bind(p);
  constrain = constrain.bind(p);
  millis = millis.bind(p);
  createCanvas = createCanvas.bind(p);
  background = background.bind(p);
